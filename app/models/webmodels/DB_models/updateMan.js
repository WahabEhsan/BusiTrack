    ...
    MongoClient.connect(con.uri, function(err, db) {
		...
		dbo.collection(con.coll).findOne(query, function(err, res) {
			...
			for (var i = con.zero; i < res.businesses.length; i++) {
				for (var j = con.zero; j < res.businesses[i].manager.length; j++) {
					if (res.businesses[i].manager[j].ssn == input[con.one]) {
						var temp = "businesses." + i.toString() + ".manager."+j.toString()+"."+input[con.zero];
